...
const streamerLegends = streamers.filter(legendas => {
    const filtered = legendas.gameMorePlayed.includes(filter);

    if (legendas.age > 35) {
        legendas.gameMorePlayed = legendas.gameMorePlayed.toUpperCase();
    }
    return filtered;
})
